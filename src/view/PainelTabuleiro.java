...
public class PainelTabuleiro extends JPanel implements Observador {
    ...
    // Controle interno para saber se o movimento foi após lançar dados.
    private boolean dadosJaLancados = false;
 
    public PainelTabuleiro() {
        setPreferredSize(new Dimension(TABULEIRO_X * 2 + TABULEIRO_LARGURA, TABULEIRO_Y * 2 + TABULEIRO_ALTURA));
    ...
    public void notify(Observado o) {
        ...
        if (tipo == GerenciadorEventos.PEAO_MOVIDO) {
            ...
            if (casaAtual != null) {
                List<String> alcancaveis = ClueFacade.getInstancia().getCasasAlcancaveis(casaAtual, new int[]{dado1, dado2});
    ...
    private void tentarMoverJogadorAtual(String destino) {
        try {
            ...
            // Movimento inválido: silencioso, conforme enunciado.
        }
    }
    ...
    private void desenharImagemTabuleiro(Graphics2D g2) {
        if (imagemTabuleiro != null) {
            g2.drawImage(imagemTabuleiro, TABULEIRO_X, TABULEIRO_Y, TABULEIRO_LARGURA, TABULEIRO_ALTURA, null);
        }
    }
